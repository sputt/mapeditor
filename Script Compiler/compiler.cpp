...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	read_commands("scriptengine.asm");
		...
		if (pass == passReplacement) {
			...
			if (str[line_num][i]) {
				...
				if (*ptr == '=') {
					...
					if (strstr(_strupr(resultbuf), "PTR") != NULL)
			...
			if (next_char(str[line_num]) > str[line_num]) { // if the first character is spaced forward, it's a command
				...
				for (i = 0; i < total_commands && _stricmp(parsebuf, commands[i]); i++);
				...
				if (i < total_commands) {
					...
					if (_stricmp(commands[i], "PARAMETERIZE") == 0) {
							...
							if (replacements_seen > 0) {
								for (int i = 0; i < replacements_seen; i++) {
									if (replacementoffsets[i] == curr_arg) {
										...
										int j;
										for (j = 0; j < total_commands && _stricmp("parameterize", commands[j]); j++);
