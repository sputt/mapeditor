...
namespace Demo
{
    ...
    public class ZcrGrammar : Irony.Parsing.Grammar
    {
        public ZcrGrammar()
        {
            ...
            statement.Rule = label | assignment | functionCall | (ToTerm("return") + ";");

            label.Rule = ToTerm("@") + identifier + ":";

            assignmentType.Rule = ToTerm("=") | ":=" | "@=";
            assignment.Rule = lvalue + assignmentType + expression + ToTerm(";");

            objectType.Rule = ToTerm("misc") | "animate" | "object" | "enemy";
            arrayAccess.Rule = objectType + "[" + simpleExpression + "]";

            structAccess.Rule = (arrayAccess | identifier) + "." + identifier;

            lvalue.Rule = structAccess | identifier | variableRef;

            term.Rule = number | identifier | "TRUE" | "FALSE" | "true" | "false";
            ...
            this.MarkReservedWords("return", "TRUE", "true", "FALSE", "false");
            #endregion
        }
    }
}
